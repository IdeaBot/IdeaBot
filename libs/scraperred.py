...
urllist.content = [x.strip() for x in urllist.content]
...
def is_new_comment(url):
    ...
    is_new = True
    return url not in data.content
        ...
        if time.time() - mostrecentrunstart >= int(config.content["period"]):
            ...
            for url in urllist.content:
                ...
                if is_new_comment(comments[0][0]):
                    ...
                    q.put([url[:-3], "https://reddit.com"+comments[0][0]])
                    data.save()
                redditLog.info("Finished scraping run in "+ str(time.time() - mostrecentrunstart))
                #except:
                #    redditLog.warning("Scraping run failed. Either the page has changed or the page is unavailable...")
