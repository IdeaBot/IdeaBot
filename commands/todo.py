    ...
    def action(self, message, send_func):
        ...
        if args==None or "list" in args.group(1).lower() or args.group(1)=="" or args.group(1)[0]=="-":
            if re.search(r'\s-p', message.content, re.I) != None:
                yield from send_func(message.channel, embed=embed.create_embed(author={"name":message.author.display_name+"'s To Do List", "url":None, "icon_url":None}, description=self.todo2string(todoFiles[message.author.id]), colour=0xffffff))
            else:
                yield from send_func(message.author, embed=embed.create_embed(author={"name":message.author.display_name+"'s To Do List", "url":None, "icon_url":None}, description=self.todo2string(todoFiles[message.author.id]), colour=0xffffff))
            return
